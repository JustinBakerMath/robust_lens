...
class ExitPlate(SubDomain):
    def inside(self, x, on_boundary):
        return (x[0] >= 0.45 and x[0]<=0.65) and x[1] == 0.0 and on_boundary
class PlateInsulation(SubDomain):
    def inside(self, x, on_boundary):
        return (x[0] < 0.45 or x[0]>0.65) and x[1] == 0.0 and on_boundary

class BoundarySource(Expression):
    def eval(self, value, x):
        if x[1] >= .95 and (x[0]<=0.02 or x[0]>=0.98):
            value[0] = -1.0
        else:
            value[0] = 0.0
...
bc = [DirichletBC(P, 1.0, ExitPlate()), DirichletBC(P, 0.0, PlateInsulation())]
f = interpolate(BoundarySource(degree=2), P, name="SourceTerm") # the volume source term for the PDE
...
# <../../documentation/optimisation>`.
    ...
    controls = File("/root/workspace/out/rob-lens/control_iterations.pvd")
...
# this and pass it to :py:mod:`pyipopt` to solve:
    ...
    xdmf_filename = XDMFFile(mpi_comm_world(), "/root/workspace/out/rob-lens/final_solution.xdmf")
